...
class ChangeMetadata_AddSimpleProperties:
    @staticmethod
    def run():
        # ExStart:1
        ...
        try:
            ...
            # Add Integer poperty
            xmp.add("xmp:Intg1", XmpValue(111))
            
            # Add DateTime poperty
            xmp.add("xmp:Date1", XmpValue(now))
            
            # Add Double poperty
            xmp.add("xmp:Double1", XmpValue(111.11))
            
            # Add String poperty
            xmp.add("xmp:String1", XmpValue("ABC"))
            
            # Save EPS file with changed XMP metadata
            
            # Create ouput stream
            with open(data_dir + "add_simple_props_output.eps", "wb") as out_ps_stream:
                # Save EPS file
